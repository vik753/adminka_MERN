...
const App = () => {
  const [messages, showMessage, deleteMessage] = useMessages(5000);
  ...
  return (
    <AuthContext.Provider
      value={{ isAuthenticated, jwtToken, userId, roles, saveLogin, logout, showMessage }}
    >
      <Router>
        <div className={styles.router_container}>
        {!!messages.length &&
          messages.map((obj, index) => <Messenger ref={messageEl} key={obj.id} params={{...obj, index, deleteMessage}} />)}
